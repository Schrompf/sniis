...
WinInput::WinInput(HWND wnd)
{
  ...
  // default-add one keyboard and one mouse to act as a global collection of all events
  auto defMouse = new WinMouse{this, mDevices.size(), nullptr};
  InputSystemHelper::AddDevice(defMouse);
  auto defKeyboard = new WinKeyboard{this, mDevices.size(), nullptr, mKeyboard};
  InputSystemHelper::AddDevice(defKeyboard);
...
void WinInput::EnumerateDevices()
{
  ...
  for( size_t a = 0; a < devices.size(); ++a ) {
    ...
    switch( dev.dwType ) {
      case RIM_TYPEMOUSE:
        try {
          InputSystem::Log("Mouse%zd: \"%s\", %d buttons, %d sample rate", mMice.size(), name.c_str(), info.mouse.dwNumberOfButtons,
      ...
      case RIM_TYPEKEYBOARD:
        try {
          InputSystem::Log("Keyboard%zd: \"%s\", %d keys", mKeyboards.size(), name.c_str(), info.keyboard.dwNumberOfKeysTotal);
        ...
        break;
      case RIM_TYPEHID:
        InputSystem::Log("HID: \"%s\", product %d, usage page %d, usage %d", name.c_str(), info.hid.dwProductId, info.hid.usUsagePage,
                         info.hid.usUsage);
...
int CALLBACK WinInput::_DIEnumDevCallback(LPCDIDEVICEINSTANCE lpddi, LPVOID pvRef)
{
  ...
  if( dt == DI8DEVTYPE_JOYSTICK || dt == DI8DEVTYPE_GAMEPAD || dt == DI8DEVTYPE_1STPERSON || dt == DI8DEVTYPE_DRIVING ||
      dt == DI8DEVTYPE_FLIGHT ) {
    try {
...
void WinInput::CheckXInputDevices()
{
  ...
  CoSetProxyBlanket(pIWbemServices, RPC_C_AUTHN_WINNT, RPC_C_AUTHZ_NONE, nullptr, RPC_C_AUTHN_LEVEL_CALL, RPC_C_IMP_LEVEL_IMPERSONATE,
                    nullptr, EOAC_NONE);
  ...
  for( ;; ) {
    ...
    for( DWORD iDevice = 0; iDevice < uReturned; iDevice++ ) {
      ...
      if( SUCCEEDED(hr) && var.vt == VT_BSTR && var.bstrVal != nullptr ) {
        // Check if the device ID contains "IG_".  If it does, then it's an XInput device - This information can not be found from
        // DirectInput
...
void WinInput::InternSetMouseGrab(bool enabled)
{
  ...
  if( enabled ) {
    ...
    pt = POINT{std::max(0, std::min(int(rect.right - rect.left), int(GetMouseX()))),
               std::max(0, std::min(int(rect.bottom - rect.top), int(GetMouseY())))};
  }
